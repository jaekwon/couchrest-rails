...
Use the rake tasks to create databases, delete databases, reset databases, push views and load fixtures:
    ...
    rake couchdb:fixtures:load[database,path]   # Load fixtures into a current environment's CouchDB database (use no database argument to use all databases defined in CouchRestRails::Document models). Default path is '/test/fixtures/couch'
    rake couchdb:fixtures:dump[database,path]   # Opposite of the load statement. 
